...
class GAIL(PPO):
    ...
    def __init__(self, buffer_exp, state_shape, action_shape, device, seed,
                 ...
                 clip_eps=0.2, lambd=0.97, coef_ent=0.0, max_grad_norm=10.0, weighted=False, state_only=False):
        super().__init__(
            state_shape, action_shape, device, seed, gamma, rollout_length,
            mix_buffer, lr_actor, lr_critic, units_actor, units_critic,
            epoch_ppo, clip_eps, lambd, coef_ent, max_grad_norm, weighted
        )
    ...
    def update(self, writer):
        ...
        # Calculate rewards.
        rewards = self.disc.calculate_reward(states, actions, log_pis)

        # Update PPO using estimated rewards.
        self.update_ppo(states, actions, rewards, dones, log_pis, next_states, writer)

    def update_disc(self, states, actions, log_pis, states_exp, actions_exp, log_pis_exp, writer):
        # Output of discriminator is (-inf, inf), not [0, 1].
        logits_pi = self.disc(states, actions, log_pis)
        logits_exp = self.disc(states_exp, actions_exp, log_pis_exp)
